...
def get_docids(cursor) -> list:
    ...
    c.execute("SELECT DISTINCT doc_id FROM entities")
    docs = c.fetchall()
    if len(docs) > 0:
        return list(docs[0])
    else:
...
def get_tfidf_terms_from_docid(cursor, doc_id: str) -> list:
    cursor.execute(
        "SELECT tfidf_terms FROM entities WHERE docid=?", (doc_id,))
    entity_tuple_list = cursor.fetchall()
    try:
...
def add_entities_to_docid(c, docid, para_id, text, entity_ids):
    ...
    c.execute('INSERT INTO entities (doc_id, para_id, text, entity_ids) VALUES (?,?,?,?)',
