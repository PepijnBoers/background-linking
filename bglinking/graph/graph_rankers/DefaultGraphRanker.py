...
from bglinking.graph.graph_rankers.InformalGraphRankerInterface import (
    InformalGraphRankerInterface,
)
...
class DefaultGraphRanker(InformalGraphRankerInterface):
    def rank(self, nodes, edges) -> dict:
        ...
        for node in nodes.values():  # initilize nodes weight by edges
            ...
            outsum_node_dict[node.name] = sum(
                (edges[edge_key] for edge_key in edges.keys() if node.name in edge_key)
            )

        sorted_keys = sorted(
            [node_name for node_name in nodes.keys()]
        ...
        for step in range(1, steps):
            ...
            for edge, weight in edges.items():
                node_a, node_b = edge
                new_weights[node_a] += (
                    weight / outsum_node_dict[node_b] * nodeweight_dict[node_b]
                )
                new_weights[node_b] += (
                    weight / outsum_node_dict[node_a] * nodeweight_dict[node_a]
                )
